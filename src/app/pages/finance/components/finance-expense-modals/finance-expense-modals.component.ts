...
import { selectShowAddForm, selectShowEditForm, selectSavingExpense, selectEditingExpense } from '../../../../store/selectors/finance.selector';
...
export class FinanceExpenseModalsComponent {
 private store = inject(Store);
  ...
  closeAdd(): void  { this.store.dispatch(FinanceActions.closeAddExpenseForm()); }
  closeEdit(): void { this.store.dispatch(FinanceActions.closeEditExpenseForm()); }

  submitAdd(): void {
    if (!this.newExpense.title.trim() || this.newExpense.amount <= 0) return;
    this.store.dispatch(FinanceActions.addExpense({
      expense: { ...this.newExpense, source: 'manual', createdAt: new Date().toISOString() },
    }));
  }
...
submitEdit(): void {
  ...
  this.store.select(selectEditingExpense).pipe(take(1)).subscribe((expense:any) => {
    if (!expense?._id) {
    ...
    this.store.dispatch(FinanceActions.updateExpense({
